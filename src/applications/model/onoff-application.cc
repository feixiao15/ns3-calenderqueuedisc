...
OnOffApplication::SendPacket()
{
    ...
    else
    {
        ...
        NS_LOG_INFO("Added FlowTypeTag to packet: " << flowTag.GetType());
    ...
    // 检查是否已有 DeadlineTag
        ...
        NS_LOG_INFO("Added DeadlineTag to packet: " << ddlTag.GetDeadline());
    ...
    // if ((unsigned)actual == m_pktSize + 2)
    // {
    //     m_txTrace(packet);
